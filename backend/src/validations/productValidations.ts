...
export class ProductValidation {
  static validate(product: Omit<ProductType,"_id">): void {
    ...
    this.validateCategory(product.category.toString()); // Convertir a string para validación
    this.validateStock(product.stock);
    this.validateImage(product.image?.toString()); // Convertir a string si está definido
  }
  ...
  private static validateImage(image?: string): void {
    if (image && !mongoose.Types.ObjectId.isValid(image))
      throw new ProductError("Image must be a valid ObjectId if provided");
  }
}
